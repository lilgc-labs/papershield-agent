...
3. 使用仓库内置 `render.yaml`：
   ...
   - `PAPERSHIELD_LLM_BASE_URL` 使用 `sync: false`，需要在 Render 环境变量界面手动填写
   - `PAPERSHIELD_LLM_MODEL` 使用 `sync: false`，需要在 Render 环境变量界面手动填写
   - `PAPERSHIELD_API_KEY` 使用 `sync: false`，需要在 Render 环境变量界面手动填写
...
PAPERSHIELD_HOSTED_FREE_RUN_LIMIT=3
PAPERSHIELD_LLM_TIMEOUT=25
PAPERSHIELD_LLM_MAX_RETRIES=0
PAPERSHIELD_PROMPT_PROFILE=default
```

3. 保存后触发一次 `Manual Deploy`，或推送一个新提交到 GitHub 触发自动部署。
4. 部署完成后访问 `/healthz`，确认 `provider` 不是 `mock`；再访问 `/api/provider/status`，确认 `configured=true`、`timeout=25`、`max_retries=0`。
5. 再访问 `/api/runtime/policy`，确认 `provider_config_enabled` 为 `true`，`admin_token_required` 为 `true`，`hosted_model_enabled` 为 `true`，`hosted_free_run_limit` 为预期次数。
